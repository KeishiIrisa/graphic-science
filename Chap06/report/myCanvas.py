...
class MyCanvas(object):  # MyCanvasクラスの定義
    ...
    def draw_grid(self, interval):

        num_x_lines = int(self.r / interval) + 1
        num_y_lines = int(self.r / interval) + 1

        # 垂直の格子線
        for i in range(-num_x_lines, num_x_lines + 1):
            x = i * interval
            self.drawPolyline(((x, -self.r), (x, self.r)))

        # 水平の格子線
        for i in range(-num_y_lines, num_y_lines + 1):
            y = i * interval
            self.drawPolyline(((-self.r, y), (self.r, y)))
